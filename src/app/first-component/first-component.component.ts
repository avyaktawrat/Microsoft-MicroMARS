...
export class FirstComponentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log(this.hGrid, this.vGrid);
  }


  fillGrey(a: number, b: number, r: number): void {
    // console.log(a,b,r);
    const element = document.getElementsByTagName('rect')[Math.floor(a / 30) * this.hGrid + Math.floor(b / 30)];
  ...
  rl_reset(): void{
    for (let i = 0; i < this.vGrid; i++) {
      for (let j = 0; j < this.hGrid; j++) {
        ...
        const element = document.getElementsByTagName('rect')[this.hGrid * i + j];
        element.style.fill = 'white';
      }
    }
    this.i = 0;
  ...
  rl_search(): void{
   ...
   // while(this.gridCord[])
    ...
    while (qu.length != 0){
      ...
      const s =   qu[0];
      qu.shift();
      const arr = this.direction_vector(s);
      for (const u of arr){
        if (!visited[u]){
          visited[u] = true;
          ...
          if (u == this.end){
            ...
            while (node != this.start){
              const element = document.getElementsByTagName('rect')[node];
              element.style.fill = 'orange';
              node = parent[node];
              this.length ++;
            }
            const milli2 = performance.now();
  ...
  shortest_path(a: number): number{
    ...
    const arr = this.direction_vector(a);
    for (const j in arr){
      if (this.gridCord[arr[j]].obstacle == 0){
        const element = document.getElementsByTagName('rect')[arr[j]];
        element.style.fill = 'yellow';
  ...
  search_A(): void {
    ...
    while (openList.length != 0) {
      ...
      // find neighbors
      const neighbors = this.direction8_vector(currentNode);
 ...
 //  }
  distance(a: number, b: number ): number {
    // console.log(a,b);
    const x1 = (a / this.hGrid);
    const y1 = a % this.hGrid;
    const x2 = (b / this.hGrid);
    const y2 = b % this.hGrid;
